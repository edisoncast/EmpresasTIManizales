...
El repositorio contiene **736 registros**. Después de la revisión preproducción, **593** se presentan como verificados, **54** como parcialmente verificados, **1** permanece por confirmar y **88** se muestran como posiblemente inactivos porque su razón social incluye “En liquidación”. No existen registros verificados sin fuente declarada.
...
|---|---:|---|
| `verified` | 593 | Fuente registrada y sin señal pendiente en el modelo actual. |
| `partially_verified` | 54 | 8 universidades, 30 programas y 16 personas con fuente, pero con verificación incompleta o pendiente de segunda revisión. |
| `unknown` | 1 | Universidad Católica de Manizales requiere confirmación adicional. |
| `inactive_or_unverified` | 88 | Empresas cuya razón social contiene “En liquidación”; se preservan y requieren revisión humana. |
| `archived` | 0 | No hay fichas formalmente archivadas todavía. |

Otros indicadores:

- **143** registros mantienen `needsVerification: true`.
- **42** registros no tienen `lastVerifiedAt`.
...
- **15** nombres empresariales se repiten. Las descripciones indican que varios pares representan persona jurídica y establecimiento de comercio, por lo que no deben fusionarse automáticamente.

## Cambios relevantes frente a 2020

- El alcance empresarial pasó de un archivo externo no reproducible a 675 registros JSON consultables y validados.
- Las 9 instituciones y los 30 programas del estudio histórico se preservan, pero se reclasifican como parciales o pendientes cuando no existe fecha de revisión 2026.
...
- El conjunto actual contiene 19 personas basadas en contribuciones públicas, sin ranking. Trece perfiles fueron incorporados durante esta revisión; se mantienen como parcialmente verificados hasta completar una revisión humana de sus afirmaciones y fuentes.
...
2. Las **54 fichas parcialmente verificadas**: completar la revisión humana y, de ser posible, registrar una URL específica por entidad o programa.
3. La **Universidad Católica de Manizales**: confirmar campos institucionales y programas antes de cambiar su estado.
4. Los **15 nombres empresariales repetidos**: determinar si son duplicados, sedes, establecimientos o entidades jurídicas distintas.
5. Los **94 negocios de persona natural**: revisar periódicamente minimización de datos y pertinencia para el ecosistema; nunca incorporar identificadores personales.
6. Las cuatro comunidades heredadas ausentes del dataset actual: decidir entre revalidar, marcar inactivas o archivar con fuente.
7. Los artefactos XLSX: documentar origen, fecha, licencia/condiciones de uso, checksum y transformación a JSON.
8. Los 13 perfiles incorporados durante la revisión: contrastar cargos, vínculos regionales y contribuciones con una segunda fuente pública antes de marcarlos como verificados.
...
- Dos registros con el mismo nombre no son duplicados automáticos cuando su forma jurídica o descripción difiere.
...
| Nombres repetidos | `MEDIUM` | No se fusionan sin evidencia; slugs siguen siendo únicos. |
| Perfiles de personas incorporados con una sola fuente | `MEDIUM` | Se muestran como `partially_verified` y requieren segunda revisión. |
...
3. Revisar las 88 empresas en liquidación por lotes documentados y las 15 coincidencias de nombre.
